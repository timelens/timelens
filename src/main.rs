...
fn main() {
    ...
    let pipeline = gst::parse_launch(&format!("filesrc location={} ! decodebin ! queue ! videoconvert ! videoscale ! video/x-raw,width=100,height=100 ! appsink emit-signals=TRUE", uri)).unwrap();
